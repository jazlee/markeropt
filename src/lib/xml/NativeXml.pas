...
type
  ...
  public
    ...
    function FindNode(const NodeName: string; const DeepFind: Boolean = true): TXmlNode;
    ...
    // Call NodeCopy to copy only current node without its child from another TXmlNode
    // into this node
    procedure NodeCopy(Source: TPersistent);
    ...
    procedure AttributeCopy(Source: TPersistent);
...
procedure TXmlNode.AttributeCopy(Source: TPersistent);
var
  i: integer;
begin
  if Source is TXmlNode then
  begin
    ...
    begin
      CheckCreateAttributesList;
      for I := 0 to (TXmlNode(Source).FAttributes.Count - 1) do
        AttributeByName[TXmlNode(Source).FAttributes.Names[I]] :=
          TXmlNode(Source).AttributeValue[I];
    end;
  end;
end;
...
procedure TXmlNode.NodeCopy(Source: TPersistent);
begin
  if Source is TXmlNode then
  begin
    // Properties
...
function TXmlNode.FindNode(const NodeName: string; const DeepFind: Boolean): TXmlNode;
// Find the first node which has name NodeName. Contrary to the NodeByName
// function, this function will search the whole subnode tree, using the
// DepthFirst method.
var
...
begin
  ...
  begin
    ...
    if AResult.CompareNodeName(NodeName) = 0 then
    begin
      Result := AResult;
      exit;
    end;
    ...
    if DeepFind then
    begin
      AResult := AResult.FindNode(NodeName);
