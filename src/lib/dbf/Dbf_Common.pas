...
begin
  ...
  if (FromCP = GetOEMCP) and (ToCP = GetACP) then
    OemToCharBuff(PAnsiChar(AnsiString(Src)), PChar(String(Dest)), Length)
  else
  if (FromCP = GetACP) and (ToCP = GetOEMCP) then
    CharToOemBuff(PChar(String(Src)), PAnsiChar(AnsiString(Dest)), Length)
  else
  if FromCP = ToCP then
  begin
    if Src <> Dest then
    ...
    wideBytes := MultiByteToWideChar(FromCP, MB_PRECOMPOSED, PAnsiChar(AnsiString(Src)), Length, LPWSTR(@WideCharStr[0]), 1024);
    WideCharToMultiByte(ToCP, 0, LPWSTR(@WideCharStr[0]), wideBytes, PAnsiChar(AnsiString(Dest)), Length, nil, nil);
  end;
end;
