...
begin
  ...
  resLen := FloatToText(PChar(@Buffer[0]), Val, {$ifndef FPC_VERSION}fvExtended,{$endif} ffFixed, Size, Precision);
...
{$endif}
      ...
      ftString:
        StrLCopy(PChar(Dst), PChar(Src), FieldSize);
    end else begin
      case DataType of
      ftString:
        if Dst <> nil then
