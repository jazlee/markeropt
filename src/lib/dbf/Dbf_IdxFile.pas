        ...
        etFloat:
          begin
            ...
            if ExtValue <> 0.0 then
              NumDecimals := StrLen(PChar(@FloatRec.Digits[0]))
            else
              NumDecimals := 0;
...
begin
  ...
  begin
    ...
    OemKey := PAnsiChar(AnsiString(FUserKey));
    if KeyType = 'C' then
    begin
      FUserKey := @lSearchKey[0];
      TranslateToANSI(OemKey, PAnsiChar(AnsiString(FUserKey)));
    end;
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        FUserKey := @lSearchKey[0];
        TranslateToANSI(PAnsiChar(AnsiString(OemKey)), PAnsiChar(AnsiString(FUserKey)));
      end;
      FindKey(false);
    end;
...
begin
  ...
  begin
    ...
    if KeyType = 'C' then
      TranslateToANSI(PAnsiChar(AnsiString(FUserKey)), PAnsiChar(AnsiString(FUserKey)));
    // find this key
    FUserRecNo := RecNo;
    FindKey(false);
  end;
...
begin
  ...
  ResetRange;
  // search lower bound
  Result := SearchKey(PChar(String(FLowBuffer)), stGreaterEqual);
  if not Result then
  begin
  ...
  SetBracketLow;
  // search upper bound
  Result := SearchKey(PChar(String(FHighBuffer)), stGreater);
...
begin
  ...
  case action of
    ...
    begin
      ...
      if (Find(recno, PChar(String(FKeyBuffer))) <> 0) then
      begin
...
var
  Key1T, Key2T: array [0..100] of AnsiChar;
  FromCP, ToCP: Integer;
begin
  ...
  begin
    ...
    Result := CompareString(FLocaleID, 0, PChar(String(Key1T)), KeyLen, PChar(String(Key2T)), KeyLen);
    if Result > 0 then
      Dec(Result, 2);
  end
end;
...
procedure TIndexFile.TranslateToANSI(Src, Dest: PAnsiChar);
begin
  { FromCP = FCodePage; }
  { ToCP = FLocaleCP;   }
...
function TIndexFile.CompareKeyString(Key: PAnsiChar): Integer;
var
  KeyT: array [0..100] of AnsiChar;
begin
  if FLocaleID = LCID(lcidBinary) then
  begin
