...
uses
   Classes, TStringUnit, TPrintWriterUnit, SysUtils;
...
begin
   ...
   for index := 0 to FValues.Count-1 do begin
      ...
      writer.print('=');
      writer.println(TString(FValues.Objects[index]).GetString);
   end;
   writer.Free;
end;

procedure TProperties.SaveToFile(const AFileName: String);
var
  AStream: TFileStream;
begin
...
procedure TProperties.LoadFromFile(const AFileName: String);
var
  AStream: TFileStream;
begin
...
begin
   index := FValues.IndexOf(AKey);
   if (index >= 0) then
      result := TString(FValues.Objects[index]).GetString
   else
