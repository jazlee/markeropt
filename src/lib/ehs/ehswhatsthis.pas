...
uses
  ...
  stdctrls, TypInfo, {$IFDEF EHS_D5_UP} ActnList, {$ENDIF} ehsbase, ehscontextmap;
...
{$ENDIF}
  ...
  private
    ...
    {$IFDEF EHS_D5_UP}
    procedure SetWTAction(action: TAction);
    {$ENDIF}
...
{   debug procedures  }
  ...
  published
    ...
    {$IFDEF EHS_D5_UP}
    property WTAction: TAction read fWTAction write SetWTAction;
    {$ENDIF}
...
{$IFDEF EHS_D5_UP}
procedure TWhatsThis.SetWTAction(action: TAction);
begin
     if IsActive then raise Exception.create('TWhatsThis must be deactivated to change this property');
     fWTAction := action;
...
begin
  ...
  opRemove:
    begin
       ...
       {$IFDEF EHS_D5_UP}
       if (AComponent = fWTAction)        then fWTAction := nil;
       {$ENDIF}
      ...
      case wP of
             ...
             begin
                 ...
                 begin
                   if Mode <> wmPermanent then Mode := wmWaitPopup
                        ...
                        if (fActiveControl = fWTToolbarButton)
                        {$IFDEF EHS_D5_UP}
                        or ((fWTAction <> nil) and (fActiveControl.action = fWTAction))
                        {$ENDIF}
                        then begin
                             CancelContextHelp;
                             Mode := wmWaitPopup;
       ...
       WM_LBUTTONDOWN:
         ...
         wmPermanent:
           begin
               fActiveControl := ControlFromPoint(HWND, pt);
               if (fWTToolbarButton = nil) or (fWTToolbarButton <> fActiveControl)
               {$IFDEF EHS_D5_UP}
               or ((fWTAction <> nil) and (fActiveControl.action <> fWTAction))
               {$ENDIF}
               then begin
       ...
       WM_LBUTTONUP:
         ...
         wmPermanent:
           ...
           begin
              if (fWTMenuItem = nil) or (fWTMenuItem <> fActiveMenuItem)
              {$IFDEF EHS_D5_UP}
              or ((fWTAction <> nil) and (fActiveMenuItem.action <> fWTAction))
              {$ENDIF}
              then begin
...
begin
  ...
  {$IFDEF EHS_D5_UP}
  if assigned(fWTAction) then fWTAction.checked := HelpMode;
  {$ENDIF}
end;
...
procedure TWhatsThis.UpdateControlEvents(Activate: boolean);
  ...
  begin
       ...
       {$IFDEF EHS_D5_UP}
       if (Control is TAction) then PropInfo := GetPropInfo(Control.classinfo, 'OnExecute');
       {$ENDIF}
...
begin
     ...
     begin
       ...
       {$IFDEF EHS_D5_UP}
       if assigned(fWTAction) then
          if not UpdateMethod(fWTAction, fWTActionExecute) then fWTAction := nil;
       {$ENDIF}
     end;
end;
