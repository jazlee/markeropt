...
type
  ...
  TCSPNTopLeastOptimizer = class(TCSPBaseOptimizer)
  private
    FStyle: TStyleItems;
    procedure PrepareNTopLeastQty;
  protected
...
constructor TCSPNTopLeastOptimizer.Create(AOwner: TComponent);
begin
  inherited Create(AOwner);
  FStyle := TStyleItems.Create(Self);
end;

procedure TCSPNTopLeastOptimizer.DeinitEngine;
begin
  inherited DeinitEngine;
end;

destructor TCSPNTopLeastOptimizer.Destroy;
begin
  FStyle.Free;
  inherited Destroy;
end;

class function TCSPNTopLeastOptimizer.EngineDescription: string;
begin
  Result := 'Optimizer based on N-Top Least quantity';
end;

class function TCSPNTopLeastOptimizer.EngineName: string;
begin
  Result := 'N-Top Least Quantity';
end;

class function TCSPNTopLeastOptimizer.EngineUniqueID: string;
begin
  Result := 'NTopLeast';
end;

function TCSPNTopLeastOptimizer.GetLockStatus(RowIndex,
  ColIndex: integer): Boolean;
begin
  Result := RowLockStatus[RowIndex];
end;

procedure TCSPNTopLeastOptimizer.InitEngine;
var
  AMarkerLen: Double;
  ARootNode: TXmlNode;
begin
...
procedure TCSPNTopLeastOptimizer.InternalDeinitDistribution;
begin
  inherited InternalDeinitDistribution;
end;

procedure TCSPNTopLeastOptimizer.InternalInitDistribution;
begin
  inherited InternalInitDistribution;
end;

procedure TCSPNTopLeastOptimizer.InternalOptimize;
var
  AMaxStep: integer;
  ALoop: Boolean;
begin
...
procedure TCSPNTopLeastOptimizer.PrepareNTopLeastQty;
...
procedure TCSPNTopLeastOptimizer.SetLockStatus(RowIndex, ColIndex: integer;
  const Value: Boolean);
begin
...
var
  AProp: TECFParams;
begin
  AProp := TCSPNTopLeastOptimizer(TStyleItem(Collection.Owner).
...
begin
  ...
  if Node.HasAttribute('rowlock') then
    AValue := Node.ReadAttributeBool('rowlock');
  TCSPNTopLeastOptimizer(TStyleItem(Collection.Owner).
...
var
  ...
  AReversed: Boolean;
begin
  AEngineProp := TCSPNTopLeastOptimizer(TStyleItem(Collection.Owner).
    Collection.Owner).EngineProperties;
  AProp := TCSPNTopLeastOptimizer(TStyleItem(Collection.Owner).
    Collection.Owner).RowProperties;
  AReversed := AProp.ParamByName('reverse').AsBoolean;
...
begin
  ...
  begin
    ANode := ARoot.Nodes[I];
    ALock := TCSPNTopLeastOptimizer(TStyleItem(Owner).Collection.Owner).
...
initialization
  RegisterOptimizer(TCSPNTopLeastOptimizer);

finalization
  UnregisterOptimizer(TCSPNTopLeastOptimizer);
