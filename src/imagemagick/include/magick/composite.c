 ...
 * WARNING this is not doing correct 'over' blend handling.
 * Anthony Thyssen
 */
  ...
  const MagickRealType Sa, const MagickRealType Dca,const MagickRealType Da)
{
  ...
  if ( Sca < MagickEpsilon )
    return( Dca*(1.0-Sa) );
  return ( Sa*Da - Sa*MagickMin(Da,(Da-Dca)*Sa/Sca) +
  ...
  const MagickRealType beta,MagickPixelPacket *composite)
{
  ...
  composite->red=gamma*
      ColorBurn(QuantumScale*p->red*Sa,Sa,QuantumScale*q->red*Da,Da);
  composite->green=gamma*
      ColorBurn(QuantumScale*p->green*Sa,Sa,QuantumScale*q->green*Da,Da);
  composite->blue=gamma*
      ColorBurn(QuantumScale*p->blue*Sa,Sa,QuantumScale*q->blue*Da,Da);
  if (q->colorspace == CMYKColorspace)
    composite->index=gamma*
      ColorBurn(QuantumScale*p->index*Sa,Sa,QuantumScale*q->index*Da,Da);
}
  ...
  const MagickRealType Sa, const MagickRealType Dca,const MagickRealType Da)
{
   ...
   * This specification was also wrong of non-overlap cases
   */
   ...
   *    f(Sc,Dc) = Dc/(1-Sc)
   * This works correctly! Looks like the 2004 model was right
   * but just required a extra condition for correct handling.
   */
  if ( fabs(Sca-Sa) < MagickEpsilon && fabs(Dca) < MagickEpsilon )
    return( Sca*(1.0-Da) + Dca*(1.0-Sa) );
  if ( fabs(Sca-Sa) < MagickEpsilon )
  ...
  const MagickRealType beta,MagickPixelPacket *composite)
{
  ...
  gamma=QuantumRange/(fabs(gamma) <= MagickEpsilon ? 1.0 : gamma);
  composite->red=gamma*
      ColorDodge(QuantumScale*p->red*Sa,Sa,QuantumScale*q->red*Da,Da);
  composite->green=gamma*
      ColorDodge(QuantumScale*p->green*Sa,Sa,QuantumScale*q->green*Da,Da);
  composite->blue=gamma*
      ColorDodge(QuantumScale*p->blue*Sa,Sa,QuantumScale*q->blue*Da,Da);
  if (q->colorspace == CMYKColorspace)
    composite->index=gamma*
      ColorDodge(QuantumScale*p->index*Sa,Sa,QuantumScale*q->index*Da,Da);
}


static inline MagickRealType Darken(const MagickRealType p,
     const MagickRealType alpha,const MagickRealType q,
  ...
  const MagickRealType Sa, const MagickRealType Dca,const MagickRealType Da)
{
  /* Divide is basically...
   *    f(Sc,Dc) = Sc/Dc
   * But with appropriate handling for special case of Dc == 0
   * Specifically f(Black,Black)  = Black
   * and       f(non-Black,Black) = White
   * It is however also important to correctly do 'over' alpha blending
   * which is why it becomes so complex looking.
   *
  ...
  const MagickRealType beta,MagickPixelPacket *composite)
{
  ...
  gamma=QuantumRange/(fabs(gamma) <= MagickEpsilon ? 1.0 : gamma);
  composite->red=gamma*
      Divide(QuantumScale*p->red*Sa,Sa,QuantumScale*q->red*Da,Da);
  composite->green=gamma*
      Divide(QuantumScale*p->green*Sa,Sa,QuantumScale*q->green*Da,Da);
  composite->blue=gamma*
      Divide(QuantumScale*p->blue*Sa,Sa,QuantumScale*q->blue*Da,Da);
  if (q->colorspace == CMYKColorspace)
    composite->index=gamma*
      Divide(QuantumScale*p->index*Sa,Sa,QuantumScale*q->index*Da,Da);
}

static MagickRealType Exclusion(const MagickRealType Sca,
  const MagickRealType Sa, const MagickRealType Dca,const MagickRealType Da)
{
  return( Sca*Da + Dca*Sa - 2.0*Sca*Dca
             + Sca*(1.0-Da) + Dca*(1.0-Sa) );
}
  ...
  const MagickRealType beta,MagickPixelPacket *composite)
{
  ...
  gamma=QuantumRange/(fabs(gamma) <= MagickEpsilon ? 1.0 : gamma);
  composite->red=gamma*
      Exclusion(QuantumScale*p->red*Sa,Sa,QuantumScale*q->red*Da,Da);
  composite->green=gamma*
      Exclusion(QuantumScale*p->green*Sa,Sa,QuantumScale*q->green*Da,Da);
  composite->blue=gamma*
      Exclusion(QuantumScale*p->blue*Sa,Sa,QuantumScale*q->blue*Da,Da);
  if (q->colorspace == CMYKColorspace)
    composite->index=gamma*
      Exclusion(QuantumScale*p->index*Sa,Sa,QuantumScale*q->index*Da,Da);
}
  ...
  const MagickRealType beta,MagickPixelPacket *composite)
{
  ...
  gamma=QuantumRange/(fabs(gamma) <= MagickEpsilon ? 1.0 : gamma);
  composite->red=gamma*
      HardLight(QuantumScale*p->red*Sa,Sa,QuantumScale*q->red*Da,Da);
  composite->green=gamma*
      HardLight(QuantumScale*p->green*Sa,Sa,QuantumScale*q->green*Da,Da);
  composite->blue=gamma*
      HardLight(QuantumScale*p->blue*Sa,Sa,QuantumScale*q->blue*Da,Da);
  if (q->colorspace == CMYKColorspace)
    composite->index=gamma*
      HardLight(QuantumScale*p->index*Sa,Sa,QuantumScale*q->index*Da,Da);
}
  ...
  const MagickRealType beta,MagickPixelPacket *composite)
{
  ...
  gamma=QuantumRange/(fabs(gamma) <= MagickEpsilon ? 1.0 : gamma);
  composite->red=gamma*
      LinearBurn(QuantumScale*p->red*Sa,Sa,QuantumScale*q->red*Da,Da);
  composite->green=gamma*
      LinearBurn(QuantumScale*p->green*Sa,Sa,QuantumScale*q->green*Da,Da);
  composite->blue=gamma*
      LinearBurn(QuantumScale*p->blue*Sa,Sa,QuantumScale*q->blue*Da,Da);
  if (q->colorspace == CMYKColorspace)
    composite->index=gamma*
      LinearBurn(QuantumScale*p->index*Sa,Sa,QuantumScale*q->index*Da,Da);
}
  ...
  const MagickRealType beta,MagickPixelPacket *composite)
{
  ...
  gamma=QuantumRange/(fabs(gamma) <= MagickEpsilon ? 1.0 : gamma);
  composite->red=gamma*
      LinearLight(QuantumScale*p->red*Sa,Sa,QuantumScale*q->red*Da,Da);
  composite->green=gamma*
      LinearLight(QuantumScale*p->green*Sa,Sa,QuantumScale*q->green*Da,Da);
  composite->blue=gamma*
      LinearLight(QuantumScale*p->blue*Sa,Sa,QuantumScale*q->blue*Da,Da);
  if (q->colorspace == CMYKColorspace)
    composite->index=gamma*
      LinearLight(QuantumScale*p->index*Sa,Sa,QuantumScale*q->index*Da,Da);
}

static inline MagickRealType Mathematics(const MagickRealType Sca,
  const MagickRealType Sa,const MagickRealType Dca,const MagickRealType Da,
  const GeometryInfo *a)
{
   ...
   *
   *  Dca' = A*Sca*Dca + B*Sca*Da + C*Dca*Sa + D*Sa*Da
   *         +  Sca*(1.0-Da) + Dca*(1.0-Sa)
   */
  return( a->rho*Sca*Dca + a->sigma*Sca*Da + a->xi*Dca*Sa + a->psi*Sa*Da
          +  Sca*(1.0-Da) + Dca*(1.0-Sa) );
     ...
     MagickPixelPacket *composite)
{
  ...
  gamma=QuantumRange/(fabs(gamma) <= MagickEpsilon ? 1.0 : gamma);
  composite->red=gamma*
      Mathematics(QuantumScale*p->red*Sa,Sa,QuantumScale*q->red*Da,Da,args);
  composite->green=gamma*
      Mathematics(QuantumScale*p->green*Sa,Sa,QuantumScale*q->green*Da,Da,args);
  composite->blue=gamma*
      Mathematics(QuantumScale*p->blue*Sa,Sa,QuantumScale*q->blue*Da,Da,args);
  if (q->colorspace == CMYKColorspace)
    composite->index=gamma*
      Mathematics(QuantumScale*p->index*Sa,Sa,QuantumScale*q->index*Da,Da,args);
}
  ...
  const MagickRealType beta,MagickPixelPacket *composite)
{
  ...
  gamma=QuantumRange/(fabs(gamma) <= MagickEpsilon ? 1.0 : gamma);
  composite->red=gamma*
      Multiply(QuantumScale*p->red*Sa,Sa,QuantumScale*q->red*Da,Da);
  composite->green=gamma*
      Multiply(QuantumScale*p->green*Sa,Sa,QuantumScale*q->green*Da,Da);
  composite->blue=gamma*
      Multiply(QuantumScale*p->blue*Sa,Sa,QuantumScale*q->blue*Da,Da);
  if (q->colorspace == CMYKColorspace)
    composite->index=gamma*
      Multiply(QuantumScale*p->index*Sa,Sa,QuantumScale*q->index*Da,Da);
}
...
/* This has been defined in "composite-private.h" no need for it here */
#if 1
#define CompositeOver(p,a,q,b,c) MagickPixelCompositeOver(p,a,q,b,c)
#else
...
static MagickRealType PegtopLight(const MagickRealType Sca,
  const MagickRealType Sa, const MagickRealType Dca, const MagickRealType Da)
{
  /* PegTOP Soft-Light Alturnative
   * A continuous version of the Softlight function, producing very similar
   * results however it does not take into account alpha channel.
   *
   * f(Sc,Dc) = Dc^2*(1-2*Sc) + 2*Sc*Dc See
   *
  ...
  const MagickRealType beta,MagickPixelPacket *composite)
{
  ...
  gamma=QuantumRange/(fabs(gamma) <= MagickEpsilon ? 1.0 : gamma);
  composite->red=gamma*
      PegtopLight(QuantumScale*p->red*Sa,Sa,QuantumScale*q->red*Da,Da);
  composite->green=gamma*
      PegtopLight(QuantumScale*p->green*Sa,Sa,QuantumScale*q->green*Da,Da);
  composite->blue=gamma*
      PegtopLight(QuantumScale*p->blue*Sa,Sa,QuantumScale*q->blue*Da,Da);
  if (q->colorspace == CMYKColorspace)
    composite->index=gamma*
      PegtopLight(QuantumScale*p->index*Sa,Sa,QuantumScale*q->index*Da,Da);
}
...
{
  /* PinLight
   * A Photoshop 7 composition method
   * http://www.simplefilter.de/en/basics/mixmods.html
   *
   * f(Sc,Dc) = Dc<2*Sc-1 ? 2*Sc-1
   *          : Dc>2*Sc   ? 2*Sc
   *                      : Dc
   *
   * Applying the SVG transparency formula (see above), we get...
  ...
  if ( Dca*Sa < Da*(2*Sca-Sa) )
    return( Sca*(Da+1.0) - Sa*Da + Dca*(1 - Sa) );
  if ( Dca*Sa > 2*Sca*Da )
  ...
  const MagickRealType beta,MagickPixelPacket *composite)
{
  ...
  gamma=QuantumRange/(fabs(gamma) <= MagickEpsilon ? 1.0 : gamma);
  composite->red=gamma*
      PinLight(QuantumScale*p->red*Sa,Sa,QuantumScale*q->red*Da,Da);
  composite->green=gamma*
      PinLight(QuantumScale*p->green*Sa,Sa,QuantumScale*q->green*Da,Da);
  composite->blue=gamma*
      PinLight(QuantumScale*p->blue*Sa,Sa,QuantumScale*q->blue*Da,Da);
  if (q->colorspace == CMYKColorspace)
    composite->index=gamma*
      PinLight(QuantumScale*p->index*Sa,Sa,QuantumScale*q->index*Da,Da);
}

/* This has been defined in "composite-private.h" no need for it here */
#if 1
#define CompositePlus(p,a,q,b,c) MagickPixelCompositePlus(p,a,q,b,c)
#else
  ...
  const MagickRealType beta,MagickPixelPacket *composite)
{
  ...
  Sa=1.0-QuantumScale*alpha;  /* simplify and speed up equations */
  Da=1.0-QuantumScale*beta;
  gamma=RoundToUnity(Sa+Da);  /* Special Plus blend, as per SVG doc */
  composite->opacity=(MagickRealType) QuantumRange*(1.0-gamma);
  /* operation performed directly - no need for a sub-routine */
  gamma=1.0/(fabs(gamma) <= MagickEpsilon ? 1.0 : gamma);
  composite->red=gamma*(p->red*Sa+q->red*Da);
  composite->green=gamma*(p->green*Sa+q->green*Da);
  composite->blue=gamma*(p->blue*Sa+q->blue*Da);
  ...
  const MagickRealType beta,MagickPixelPacket *composite)
{
  ...
  gamma=QuantumRange/(fabs(gamma) <= MagickEpsilon ? 1.0 : gamma);
  composite->red=gamma*
      Screen(QuantumScale*p->red*Sa,QuantumScale*q->red*Da);
  composite->green=gamma*
      Screen(QuantumScale*p->green*Sa,QuantumScale*q->green*Da);
  composite->blue=gamma*
      Screen(QuantumScale*p->blue*Sa,QuantumScale*q->blue*Da);
  if (q->colorspace == CMYKColorspace)
    composite->index=gamma*
      Screen(QuantumScale*p->index*Sa,QuantumScale*q->index*Da);
}
  ...
  const MagickRealType Sa, const MagickRealType Dca, const MagickRealType Da)
{
   ...
   */
  if ( 2.0*Sca < Sa )
    return( Dca*(Sa - (1.0-Dca/Da)*(2.0*Sca-Sa)) +
             Sca*(1.0-Da) + Dca*(1.0-Sa) );
  if ( 8.0*Dca <= Da )
    return( Dca*(Sa-(1.0-Dca/Da)*(2.0*Sca-Sa)*(3.0-8.0*Dca/Da)) +
             Sca*(1.0-Da) + Dca*(1.0-Sa) );
  return( (Dca*Sa + (pow(Dca/Da,0.5)*Da - Dca)*(2.0*Sca-Sa)) +
             Sca*(1.0-Da) + Dca*(1.0-Sa) );
#else
  /* New Specification  March 2009 SVG specification */
  MagickRealType
  ...
  if ( 2.0*Sca < Sa )
    return( Dca*(Sa + (2.0*Sca-Sa)*(1.0-m)) +
             Sca*(1.0-Da) + Dca*(1.0-Sa) );
  if ( 2.0*Sca > Sa && 4.0*Dca <= Da )
      return( Dca*Sa + Da*(2.0*Sca-Sa)*(4.0*m*(4.0*m+1.0)*(m-1.0)+7.0*m) +
               Sca*(1.0-Da) + Dca*(1.0-Sa) );
  return( Dca*Sa + Da*(2.0*Sca-Sa)*(pow(m,0.5)-m) +
             Sca*(1.0-Da) + Dca*(1.0-Sa) );
#endif
}
  ...
  const MagickRealType beta,MagickPixelPacket *composite)
{
  ...
  gamma=QuantumRange/(fabs(gamma) <= MagickEpsilon ? 1.0 : gamma);
  composite->red=gamma*
      SoftLight(QuantumScale*p->red*Sa,Sa,QuantumScale*q->red*Da,Da);
  composite->green=gamma*
      SoftLight(QuantumScale*p->green*Sa,Sa,QuantumScale*q->green*Da,Da);
  composite->blue=gamma*
      SoftLight(QuantumScale*p->blue*Sa,Sa,QuantumScale*q->blue*Da,Da);
  if (q->colorspace == CMYKColorspace)
    composite->index=gamma*
      SoftLight(QuantumScale*p->index*Sa,Sa,QuantumScale*q->index*Da,Da);
}
  ...
  const MagickRealType amount)
{
  ...
  diff=p-q;
  if ((MagickRealType) fabs((double) (2.0*diff)) < threshold)
      return(q);
  return(q+diff*amount);
}
  ...
  const MagickRealType beta,MagickPixelPacket *composite)
{
  ...
  gamma=QuantumRange/(fabs(gamma) <= MagickEpsilon ? 1.0 : gamma);
  composite->red=gamma*
      VividLight(QuantumScale*p->red*Sa,Sa,QuantumScale*q->red*Da,Da);
  composite->green=gamma*
      VividLight(QuantumScale*p->green*Sa,Sa,QuantumScale*q->green*Da,Da);
  composite->blue=gamma*
      VividLight(QuantumScale*p->blue*Sa,Sa,QuantumScale*q->blue*Da,Da);
  if (q->colorspace == CMYKColorspace)
    composite->index=gamma*
      VividLight(QuantumScale*p->index*Sa,Sa,QuantumScale*q->index*Da,Da);
}
  ...
  const MagickRealType beta,MagickPixelPacket *composite)
{
  ...
  composite->opacity=(MagickRealType) QuantumRange*(1.0-gamma);
  /* optiomised by multipling QuantumRange taken from gamma */
  gamma=1.0/(fabs(gamma) <= MagickEpsilon ? 1.0 : gamma);
  ...
  */
  ...
  switch (compose)
  {
    ...
    case BlurCompositeOp:
    {
      ...
      MagickRealType
        blur_xu,blur_xv,blur_yu,blur_yv;
      ...
      for (y=0; y < (long) composite_image->rows; y++)
      {
        ...
        for (x=0; x < (long) composite_image->columns; x++)
        {
          ...
          ScaleResampleFilter(resample_filter,
               blur_xu*QuantumScale*p->red,
               blur_yu*QuantumScale*p->green,
               blur_xv*QuantumScale*p->red,
      ...
      */
      ...
      if ((flags & (WidthValue|HeightValue)) == 0 )
        if ((flags & AspectValue) == 0)
          {
            horizontal_scale=(MagickRealType) (composite_image->columns-1)/2.0;
            vertical_scale=(MagickRealType) (composite_image->rows-1)/2.0;
          }
        else
          {
          ...
          if ((flags & PercentValue) != 0)
            {
              if ((flags & AspectValue) == 0)
                {
                  horizontal_scale *= (composite_image->columns-1) / 200.0;
                  vertical_scale *= (composite_image->rows-1) / 200.0;
                }
              else
                {
    ...
    case MathematicsCompositeOp:
    {
      /*
        ...
        Unused values are set to zero automatically.
      ...
      */
      ...
      switch (compose)
      {
        ...
        { /* Overlay() = reversed HardLight() */
          CompositeHardLight(&destination,destination.opacity,
