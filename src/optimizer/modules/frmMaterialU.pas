...
procedure TfrmMaterial.actCloseExecute(Sender: TObject);
begin
  Close;
end;

procedure TfrmMaterial.actDeleteExecute(Sender: TObject);
begin
...
procedure TfrmMaterial.ActionListStateChange(Sender: TObject);
begin
...
procedure TfrmMaterial.actPrintExecute(Sender: TObject);
var
  AValue : Variant;
begin
  AValue := VarArrayOf([
...
procedure TfrmMaterial.actReff_1Execute(Sender: TObject);
begin
  TfrmMaterialCat.DoBrowse;
end;

procedure TfrmMaterial.actSaveExecute(Sender: TObject);
begin
...
constructor TfrmMaterial.Create(AOwner: TComponent);
begin
...
procedure TfrmMaterial.cxDBButtonEdit1PropertiesButtonClick(Sender: TObject;
  AButtonIndex: Integer);
var
  AValue: Variant;
begin
  AValue := TfrmMaterialCat.DoBrowse('catcd');
  DataSource.DataSet.FieldByName('mtcatcd').Value := AValue;
end;

destructor TfrmMaterial.Destroy;
begin
  DataSource.DataSet.Free;
  inherited Destroy;
end;

class procedure TfrmMaterial.DoBrowse;
begin
  with TfrmMaterial.Create(Application.MainForm) do
  try
...
class function TfrmMaterial.DoBrowse(const AFields: string): Variant;
begin
  with TfrmMaterial.Create(Application.MainForm) do
  try
...
procedure TfrmMaterial.DoExternalDelete(ADataset: TDataSet);
begin
...
procedure TfrmMaterial.DoExternalEdit(ADataset: TDataSet);
begin
  ADataset.Edit;
...
procedure TfrmMaterial.DoExternalNew(ADataset: TDataSet);
begin
  ADataset.Append;
...
procedure TfrmMaterial.DoExternalPrint(ADataset: TDataSet);
var
  AValue : Variant;
begin
...
function TfrmMaterial.GetLookupValues(const AFields: string): Variant;
var
...
procedure TfrmMaterial.InitData;
begin
  DataSource.DataSet := dmReferences.GetACRTable('materials');
