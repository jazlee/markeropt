...
var
  frmLayingRule: TfrmLayingRule;

implementation

uses
  ECFUtils, dmReferencesU, dmMainU, dmResU, CSPConsts, frmBrowserU, frmLayingRuleItemsU;

{$R *.dfm}

{ TfrmLayingRule }

procedure TfrmLayingRule.actCloseExecute(Sender: TObject);
begin
  Close;
end;

procedure TfrmLayingRule.actDeleteExecute(Sender: TObject);
begin
...
procedure TfrmLayingRule.ActionListStateChange(Sender: TObject);
begin
...
procedure TfrmLayingRule.actPrintExecute(Sender: TObject);
var
  AValue : Variant;
begin
  AValue := VarArrayOf([
...
procedure TfrmLayingRule.actReff_1Execute(Sender: TObject);
begin
  TfrmLayingRuleItems.DoBrowse('lrname',DataSource.DataSet.FieldValues['lrname']);
end;

procedure TfrmLayingRule.actSaveExecute(Sender: TObject);
begin
...
constructor TfrmLayingRule.Create(AOwner: TComponent);
begin
...
procedure TfrmLayingRule.DeleteItems;
var
  ADataItem: TDataSet;
begin
  ADataItem := dmReferences.GetACRTable('itmlayrule');
  try
    ADataItem.Filter := Format('lrname = %s',
      [
...
destructor TfrmLayingRule.Destroy;
begin
  DataSource.DataSet.Free;
  inherited Destroy;
end;

class procedure TfrmLayingRule.DoBrowse;
begin
  with TfrmLayingRule.Create(Application.MainForm) do
  try
...
class function TfrmLayingRule.DoBrowse(const AFields: string): Variant;
begin
  with TfrmLayingRule.Create(Application.MainForm) do
  try
...
procedure TfrmLayingRule.DoExternalDelete(ADataset: TDataSet);
begin
...
procedure TfrmLayingRule.DoExternalEdit(ADataset: TDataSet);
begin
  ADataset.Edit;
...
procedure TfrmLayingRule.DoExternalNew(ADataset: TDataSet);
begin
  ADataset.Append;
...
procedure TfrmLayingRule.DoExternalPrint(ADataset: TDataSet);
var
  AValue : Variant;
begin
  AValue := RPT_LAYRULE_ALL_ID;
  SendMessage(Application.MainForm.Handle, WM_APPMSG, CMD_EXECPRINT,
    Integer(@AValue));
end;

procedure TfrmLayingRule.FormClose(Sender: TObject; var Action: TCloseAction);
begin
...
function TfrmLayingRule.GetLookupValues(const AFields: string): Variant;
var
...
procedure TfrmLayingRule.InitData;
begin
  DataSource.DataSet := dmReferences.GetACRTable('layrule');
